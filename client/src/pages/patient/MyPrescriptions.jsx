...
export default function MyPrescriptions() {
  ...
  const handleDownload = async (p) => {
    ...
    try {
      ...
      await downloadPrescriptionPdf({
        prescription: p,
        doctor: { name: p.doctor?.name, ...(p.doctorMeta || {}) },
        patient: { name: user.name, phone: user.phone, city: user.city },
      });
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader title="My prescriptions" subtitle="Medicines and advice from your doctors. Download any as a PDF." />

      {loading ? (
        <PageLoader />
      ) : items.length === 0 ? (
        <EmptyState icon={Pill} title="No prescriptions yet" message="When a doctor writes you a prescription, it will appear here." />
      ) : (
